...
package Stdh is
   ...
   package Unbounded_Bounded_Vectors is

      subtype Extended_Index is Index_Type'Base range 0 .. Index_Type'Last;

      type Vector is new Limited_Controlled with private;
      ...
      function Element (This  : Vector;
                        Idx   : Index_Type) return Element_Type;
      ...
      function Length (This : Vector) return Nat32;

   private

      type Elements_Array is array (Index_Type) of aliased Element_Type;
      --  The array type is aliased to allow references to individual
      --  elements in the array.

      type Elements_Ptr is access Elements_Array;

      type Vector is new Limited_Controlled with record
         Items   : Elements_Ptr;
